import type { TFunction } from "i18next";
import { z } from "zod";

export const schema = (t: TFunction<["auth"], undefined>) => {
  return z.object({
    username: z
      .string().trim()
      .nonempty({ message: t("Please enter your name", { ns: "auth" }) })
      .min(4, t("Please enter at least 4 characters", { ns: "auth" })),
    position: z
      .string().trim()
      .nonempty({ message: t("Please enter your title", { ns: "auth" }) })
      .min(3, t("Please enter at least 3 characters", { ns: "auth" })),
    email: z
      .string().trim()
      .nonempty({
        message: t("Please provide your work email address", { ns: "auth" }),
      })
      .email({
        message: t("Please enter a valid email address", { ns: "auth" }),
      }),
    phoneNumber: z
      .string().trim()
      .nonempty({
        message: t("Please provide your phone number", { ns: "auth" }),
      })
      .regex(/^(0[1-9][0-9]{8,9})$/, {
        message: t("Please enter a valid phone number", { ns: "auth" }),
      }),
    companyType: z.string().trim().nonempty({
      message: t("Please enter your company type", { ns: "auth" }),
    }),
    tagline: z.string().trim().optional(),
    industryId: z.union([
      z.string().trim().nonempty({
        message: t("Please enter your company industry", { ns: "auth" }),
      }),
      z.number(),
    ]),
    companySize: z.string().trim().nonempty({
      message: t("Please enter your company size", { ns: "auth" }),
    }),
    country: z
      .string().trim()
      .nonempty({ message: t("Please enter your country", { ns: "auth" }) }),
    workingDay: z.string().trim().nonempty({
      message: t("Please enter your working days", { ns: "auth" }),
    }),
    overtimePolicy: z.string().trim().nonempty({
      message: t("Please enter your overtime policy", { ns: "auth" }),
    }),
    companyName: z
      .string().trim()
      .nonempty({
        message: t("Please enter your company name", { ns: "auth" }),
      })
      .min(4, t("Please enter at least 4 characters", { ns: "auth" })),
    location: z
      .string().trim()
      .nonempty({ message: t("Please select a city", { ns: "auth" }) }),
    website: z
      .string().trim()
      .optional()
      .refine(
        (value) =>
          !value ||
          /^(https?:\/\/)([\w-]+(\.[\w-]+)+)(\/[\w-./?%&=]*)?$/.test(value),
        {
          message: t("Please enter a valid URL, i.e https://itviec.com", {
            ns: "auth",
          }),
        }
      ),
    skillIds: z.union([z.string(), z.array(z.number())]).optional(),
    overview: z.string().trim().optional(),
    perks: z.string().trim().optional(),
    logo: z.any().optional(),
  });
};

export const schemaChangePassword = (t: TFunction) => {
  return z
    .object({
      currentPassword: z
        .string()
        .nonempty({ message: t("Please enter your current password") }),
      newPassword: z
        .string()
        .nonempty({ message: t("Please enter your new password") })
        .min(12, t("Minimum 12 characters"))
        .regex(
          /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#$%^&*()_+~`|}{[\]\\:;?><,./-=]).{12,}$/,
          t(
            "At least 1 symbol, 1 number, 1 uppercase letter, 1 lowercase letter."
          )
        ),
      confirmPassword: z
        .string()
        .nonempty({ message: t("Please re-enter your new password") })
        .min(12, t("Minimum 12 characters")),
    })
    .superRefine(({ confirmPassword, newPassword }, ctx) => {
      if (confirmPassword !== newPassword) {
        ctx.addIssue({
          code: "custom",
          message: t("Please re-enter new password correctly"),
          path: ["confirmPassword"],
        });
      }
    });
};
